 ...
 * @param { string } arguments.backgroundColor - background color
 * 
 * @returns { string } - return SVG string
 */
exports.createFinalSVG = ( { header, body, footer, cornerRadius, backgroundColor } ) => {
    ...
    const padding = 20;
    const svgWidth = dimensions.body.width + padding * 2; // maximum width
    const svgHeight = dimensions.header.height + dimensions.body.height + dimensions.footer.height + padding * 2;
    const shadowDeviation = 4;

    return `
    <svg xmlns="http://www.w3.org/2000/svg" width="${svgWidth}" height="${svgHeight}" viewBox="0, 0, ${svgWidth}, ${svgHeight}">
    <defs>
    <filter id="shadow">
        <feDropShadow dx="0" dy="0" stdDeviation="${shadowDeviation}"/>
    </filter>
    </defs>
    <rect width="100%" height="100%" fill="${backgroundColor}"/>
    <g transform="translate(${padding}, ${padding})" style="filter:url(#shadow);">
        <clipPath id="clipPath">
            <rect x="0" y="0" width="${svgWidth - padding * 2}" height="${svgHeight - padding * 2}" rx="$cornerRadius}" ry="${cornerRadius}" />
        </clipPath>
