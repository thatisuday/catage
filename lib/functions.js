 ...
 * @returns { Promise<string> } - return execution result 
 */
exports.getExecutionResultSVG = async ( { inputFilePath, execute, displayCommand, scale, width } ) => {
    ...
    const resultSvg = codeToSvg( { fontSize: 12, code: terminalOutput, padding: '30,0', scale, ignoreLineNumbers: true } );

    // wrap `resultSvg` in terminal window
    return getTerminalResultSvg( resultSvg, width );
...
exports.getOsxWindowSvg = ( width, title ) => {
    return `
        <svg viewBox="0, 0, ${ width }, 30" font-family="monospace" font-size="14">
            ...
            <text x="50%" y="19" fill="#666666" font-weight="bold" text-anchor="middle">${ _.toUpper( title ) }</text>
        </svg>
    `;
}
 ...
 * @returns { string } - SVG string
 */
const getTerminalResultSvg = exports.getTerminalResultSvg = ( svg, width ) => {

    // original SVG dimensions
    const dimensions = getSvgDimensions( svg );

    // total SVG height
    const svgHeight = dimensions.height + 31 + 20; // 31px top offset, 10px top spacing and 10px bottom spacing
