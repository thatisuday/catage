 ...
 * 
 * @returns { Promise<Buffer> } - image buffer of the patch
 */
...
/**
 * @desc Get SVG image as clipping mask to add border radius to the output image.
 * 
 ...
 */
exports.getClippingMask = ( radius, dimensions ) => {
    const svg = `
        <svg style="b" viewBox="0 0 ${ dimensions.width } ${ dimensions.height }">
            <rect
                x="0"
                y="0"
...
/**
 * @desc Prepare nested SVG by adding dimensions and removing unnecessary attributes.
 * 
...
/**
 * @desc Return OSX window frame SVG
 * 
 ...
 * @returns { string } - SVG string
 */
exports.getWindowFrame = ( width, title = 'running code' ) => {
    const svg = `
